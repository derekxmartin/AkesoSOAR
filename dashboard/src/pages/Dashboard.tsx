    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="bg-slate-800/50 rounded-lg border border-slate-700 p-6">
          <h3 className="text-sm font-medium text-slate-400 mb-1">Use Cases</h3>
          <p className="text-3xl font-bold text-white">{ucData?.total ?? "—"}</p>
        </div>
        <div className="bg-slate-800/50 rounded-lg border border-slate-700 p-6">
          <h3 className="text-sm font-medium text-slate-400 mb-1">Executions</h3>
          <p className="text-3xl font-bold text-white">{execData?.total ?? "—"}</p>
        </div>
        <div className="bg-slate-800/50 rounded-lg border border-slate-700 p-6">
          <h3 className="text-sm font-medium text-slate-400 mb-1">Alerts Ingested</h3>
          <p className="text-3xl font-bold text-white">{alertData?.total ?? "—"}</p>
        </div>
      </div>
            ...
            <div className="space-y-2">
              {execData.items.map((e: any) => (
                <div key={e.id} className="flex items-center justify-between text-sm">
                  <span className="text-slate-300 font-mono text-xs">{e.id.slice(0, 8)}...</span>
