        ...
        <table className="w-full text-sm">
          ...
          <tbody>
            {isLoading ? (
              ...
              <tr><td colSpan={5} className="px-4 py-8 text-center text-slate-400">No executions found</td></tr>
            ) : (
              data.items.map((ex: Execution) => (
                ...
                >
                  <td className="px-4 py-3 font-mono text-xs text-slate-300">{ex.id.slice(0, 12)}...</td>
                  <td className="px-4 py-3"><Badge value={ex.status} /></td>
                  <td className="px-4 py-3 text-slate-300">{ex.trigger_alert_id || "manual"}</td>
                  <td className="px-4 py-3 text-slate-300">{ex.duration_ms != null ? `${ex.duration_ms}ms` : "—"}</td>
                  <td className="px-4 py-3 text-slate-400">{ex.started_at ? new Date(ex.started_at).toLocaleString() : "—"}</td>
                </tr>
              ))
            )}
          </tbody>
