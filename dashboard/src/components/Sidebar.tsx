...
export default function Sidebar({ open, onToggle }: SidebarProps) {
  return (
    <>
      ...
      <aside
        className={cn(
          "fixed top-0 left-0 z-40 h-full w-64 bg-slate-900 border-r border-slate-700 transition-transform lg:translate-x-0 lg:static lg:z-auto",
          open ? "translate-x-0" : "-translate-x-full"
        )}
      >
        <div className="flex items-center justify-between h-16 px-4 border-b border-slate-700">
          <span className="text-lg font-bold text-white">AkesoSOAR</span>
