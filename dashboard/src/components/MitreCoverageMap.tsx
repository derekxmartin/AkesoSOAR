...
const COVERAGE_COLORS: Record<string, string> = {
  covered: "bg-green-600 hover:bg-green-500",
  partial: "bg-yellow-600 hover:bg-yellow-500",
  gap: "bg-red-900/40 hover:bg-red-900/60",
};
...
export default function MitreCoverageMap() {
  ...
  const allTactics = Object.entries(data.matrix);
    ...
    <div className="space-y-4">
      ...
      <div className="overflow-x-auto pb-2">
        <div className="flex gap-2 min-w-max">
          {allTactics.map(([tacticId, tactic]) => (
            <div key={tacticId} className="flex flex-col gap-1 min-w-[120px]">
              {/* Tactic header */}
              <div className="text-[10px] font-semibold text-fg3 uppercase tracking-wide px-1 py-1.5 border-b border-edge mb-1 text-center">
                {tactic.tactic_name}
                <div className="text-[9px] text-fg4 font-normal mt-0.5">
              ...
              {tactic.techniques.map((tech) => (
                <button
                  ...
                  className={`px-2 py-1 rounded text-[9px] text-white/90 text-left transition-colors ${
                    COVERAGE_COLORS[tech.coverage] || COVERAGE_COLORS.gap
                  } ${selectedTech?.technique_id === tech.technique_id ? "ring-2 ring-blue-400" : ""}`}
                >
                  <div className="font-mono leading-tight">{tech.technique_id}</div>
                </button>
              ))}
            </div>
          ))}
      ...
      {tooltipTech && (
        ...
        >
          ...
          <div className="text-fg4 mt-1">
            ...
            {" · "}
            <span className={tooltipTech.tech.coverage === "covered" ? "text-green-400" : tooltipTech.tech.coverage === "partial" ? "text-yellow-400" : "text-red-400"}>
